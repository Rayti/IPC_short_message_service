...
int main(int argc, char* argv[]){
    ...
    if(requestQueue == -1){
        printf("Error in creating requestQueue!\n");
    }
    if(messageQueue == -1){
        printf("Error in creatin messageQueue!\n");
    }
    while(1){
        ...
        caseGroupSigning(users, usersAmount, groups, groupsAmount);
        caseSigningOutOfGroup(users, usersAmount);
        caseGroupList(groups, groupsAmount);
    }
}
